...
| POST | `/auth/password-reset/request` | 비밀번호 재설정 코드 발송 | — |
| POST | `/auth/password-reset/verify` | 인증번호 확인 (코드를 쓰지 않음) | — |
| POST | `/auth/password-reset/confirm` | 코드 확인 후 비밀번호 변경 | — |
...
| 비밀번호 변경 | **`users.password_changed_at` 이전에 발급된 토큰은 access·refresh 모두 `401`** — 비밀번호를 바꾸면 기존 세션이 전부 끊긴다 |
...
비밀번호를 잊었을 때 6자리 인증번호를 메일로 보냅니다.
...
**가입 여부와 무관하게 항상 `202`입니다.** 없는 이메일에 `404`를 주면 그 응답만으로
가입자 목록을 훑을 수 있습니다. 가입돼 있든, 소셜 계정이든, 시간당 상한을 넘겼든
호출한 쪽에 보이는 것은 똑같습니다.

발급과 발송은 **뒷작업**으로 넘깁니다. 메일 서버가 느릴 때 응답이 같이 늦어지면
그 시간 차이만으로 "가입된 이메일"이 드러나고, 앱도 그만큼 멈춥니다.
...
응답은 모두 같은 `202`이고, 서버가 조용히 건너뜁니다.

| 상황 | 이유 |
| --- | --- |
| 가입되지 않은 이메일 | 알려줄 것이 없습니다 |
| 탈퇴한 계정(`deleted_at`) | 탈퇴 계정이 재설정으로 되살아나면 안 됩니다 |
| 소셜 계정(`auth_provider ≠ local`) | 비밀번호를 만들어 주면 **원래 없던 로그인 경로가 새로 생깁니다.** 메일함만 뚫려도 계정이 넘어갑니다. 대신 "카카오로 로그인하세요" 안내 메일을 보냅니다 |
| 시간당 상한 초과(기본 5회) | 남의 메일함에 코드 메일을 쏟아붓지 못하게 막습니다 |

---

## POST /auth/password-reset/verify

인증번호가 맞는지만 확인합니다. **코드를 소모하지 않습니다** — 통과한 뒤에
`confirm`을 이어서 부릅니다.

### 왜 확인만 하는 단계가 따로 있나

앱이 인증번호와 새 비밀번호를 **두 화면으로 나눠** 받기 때문입니다. 한 화면에서
같이 받으면, 코드가 틀렸다는 것을 새 비밀번호까지 다 입력한 뒤에야 알게 됩니다.

### 요청

```json
{ "email": "traveler@example.com", "code": "482913" }
```

### 응답 `204`

본문 없음. 코드는 그대로 살아 있습니다(`used_at`을 채우지 않습니다).

### 에러

`confirm`과 **응답 모양이 같습니다.** 두 엔드포인트의 응답이 갈리면 그 차이만으로
코드가 맞았는지 짐작할 수 있습니다.

| 코드 | 상황 |
| --- | --- |
| 400 | 코드 없음·만료·이미 사용·불일치 (**구분하지 않습니다**), 가입되지 않은 이메일, 소셜 계정 |
| 429 | 입력 시도 상한 초과 (해당 코드는 폐기됩니다) |
| 422 | 이메일 형식 오류, 6자리 숫자가 아님 |

### 맞았을 때는 시도 횟수를 세지 않습니다 [확정]

여기서 한 번 세고 이어지는 `confirm`에서 또 세면, 정상 사용자가 재설정 한 번에
시도를 2회 씁니다 — 오타를 몇 번 낸 사람이 **맞는 코드를 넣고도** 상한에 걸려
잠깁니다. **틀린 입력은 그대로 셉니다.** 안 세면 이 엔드포인트가 상한 없는
무차별 대입 창구가 됩니다.
...
받은 인증번호를 확인하고 비밀번호를 바꿉니다. **`verify`를 통과했다는 사실을
믿지 않고 코드를 다시 검사합니다** — `verify`를 건너뛰고 바로 올 수 있고, 두
화면 사이에 코드가 만료(10분)됐을 수도 있습니다.
...
| 필드 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `email` | string | ✅ | 코드를 받은 이메일 |
| `code` | string | ✅ | **6자리 숫자.** 형식이 어긋나면 코드 대조 없이 `422` |
| `newPassword` | string | ✅ | 회원가입과 같은 규칙 — **최소 8자·최대 128자.** 여기만 느슨하면 재설정이 비밀번호 규칙의 우회로가 됩니다 |

### 응답 `204`

본문 없음. **토큰을 발급하지 않습니다** — 재설정 직후 이전 토큰을 전부 무효로
만드는 참에 새 토큰까지 여기서 내주면 경계가 헷갈립니다. 앱은 바뀐 비밀번호로
로그인 화면을 한 번 거칩니다.
...
| 코드 | 상황 |
| --- | --- |
| 400 | 인증번호가 없음·만료됨·이미 사용됨·불일치 (**넷을 구분하지 않습니다** — 어느 쪽인지 알려주면 유효한 코드를 찾는 힌트가 됩니다) |
| 429 | 입력 시도 상한 초과. 이건 구분해서 알려줍니다 — 안 알려주면 사용자가 죽은 코드를 계속 입력합니다 |
| 422 | 코드가 6자리 숫자가 아니거나 비밀번호 규칙 미달 |

### 성공하면 기존 세션이 전부 끊깁니다 [확정]

`users.password_changed_at`을 지금 시각으로 채우고, **그 이전에 발급된 access·refresh
토큰을 전부 `401`로 막습니다.**

이 처리가 없으면 계정을 되찾아 비밀번호를 바꿔도 공격자가 이미 받아둔 refresh token
으로 **최대 14일** 재발급을 이어갑니다. 그러면 비밀번호 재설정이 존재하는 이유
(계정 탈취 대응)가 반쯤 무너집니다.

### 무차별 대입 방어

6자리는 100만 조합뿐이라 시도 제한이 유일한 방어입니다.

| 장치 | 기본값 | 설정 |
| --- | --- | --- |
| 코드 유효 시간 | 10분 | `PASSWORD_RESET_CODE_TTL_MINUTES` |
| 코드당 입력 시도 | 5회 | `PASSWORD_RESET_MAX_ATTEMPTS` |
| 계정당 시간당 발송 | 5회 | `PASSWORD_RESET_HOURLY_LIMIT` |

시도 횟수는 **틀린 즉시 커밋**합니다. 에러로 빠져나가며 증가분까지 롤백되면
횟수 제한이 통째로 무력해집니다.

살아 있는 코드는 회원당 최대 하나입니다 — 새 코드를 발급할 때 이전 것들을 함께
폐기합니다. 여러 개가 동시에 살아 있으면 맞춰볼 수 있는 조합이 그만큼 늘어납니다.

### 메일 발송

인증번호는 SMTP 로 보냅니다. 발송 지점은
[`services/email.py`](../../apps/api/app/services/email.py) 한 곳이고, 재설정
로직은 "보낸다"만 호출합니다.

| 설정 | 기본값 | 설명 |
| --- | --- | --- |
| `SMTP_HOST` | (없음) | 네이버는 `smtp.naver.com` |
| `SMTP_PORT` | `465` | 465 = 접속부터 SSL, 587 = 접속 후 STARTTLS |
| `SMTP_USE_SSL` | `true` | 587 을 쓸 때만 `false` |
| `SMTP_USERNAME` | (없음) | 로그인 계정 |
| `SMTP_PASSWORD` | (없음) | 로그인 비밀번호가 아니라 **애플리케이션 비밀번호** |
| `SMTP_FROM_EMAIL` | `SMTP_USERNAME` | 로그인 계정과 다른 주소는 대개 거절됩니다 |
| `SMTP_FROM_NAME` | `오멍가멍` | 받는 사람에게 보이는 발신자 이름 |
| `SMTP_TIMEOUT_SECONDS` | `10` | 무제한으로 두면 백그라운드 스레드가 잡힌 채 남습니다 |

**HOST·USERNAME·PASSWORD 중 하나라도 비면 발송을 건너뛰고 로그만 남깁니다.**
설정이 없다고 기동을 막지 않는 이유는, 팀원 로컬과 CI 에 메일 계정을 나눠 주지
않기 때문입니다 — 막으면 메일과 무관한 작업을 하는 사람의 서버가 안 뜹니다.
`ENVIRONMENT=local` 이고 발송 수단이 없을 때만 제목·본문을 통째로 로그에
남깁니다(터미널에서 인증번호를 봐야 하므로). 그 외에는 수신자와 제목만 남깁니다 —
실제로 메일이 나가는데 로그에도 코드가 남으면, 로그를 볼 수 있는 사람이 남의
계정 비밀번호를 바꿀 수 있습니다.

발송이 실패해도 예외를 밖으로 던지지 않습니다. 실패가 응답에 드러나면 그것만으로
가입 여부를 알아낼 수 있고, 애초에 뒷작업이라 사용자에게 전달할 방법도 없습니다.
...
| 2026-09-02 | **비밀번호 재설정 추가** — `POST /auth/password-reset/request`·`verify`·`confirm`. 앱은 인증번호와 새 비밀번호를 두 화면으로 나눠 받는다(한 화면이면 코드가 틀렸다는 것을 비밀번호까지 다 입력한 뒤에 알게 된다). `verify` 는 코드를 소모하지 않고, **맞았을 때는 시도 횟수도 세지 않는다** — 이어지는 `confirm` 이 한 번 세므로 여기서도 세면 정상 사용자가 한 번의 재설정에 시도 2회를 쓴다. 6자리 코드를 메일로 보내고 `password_reset_codes` 에 해시로 보관한다(코드를 토큰에 실으면 JWT 페이로드가 열람 가능해 6자리가 오프라인에서 즉시 뚫린다). 소셜·탈퇴 계정은 발급하지 않고, 응답은 가입 여부와 무관하게 항상 202. **`users.password_changed_at` 을 함께 도입해 비밀번호 변경 이전 발급 토큰을 access·refresh 모두 무효화**한다(없으면 계정을 되찾아도 공격자가 refresh 수명 14일을 그대로 쓴다). 메일 발송은 아직 로그 출력 스텁 — 실제 전송 수단은 별도 PR |
| 2026-09-02 | **인증번호 메일 실제 발송** — `SMTP_*` 설정을 추가하고 [`services/email.py`](../../apps/api/app/services/email.py) 의 `_deliver` 를 SMTP 전송으로 교체. 재설정 로직·엔드포인트·스키마는 그대로다. `SMTP_HOST`·`SMTP_USERNAME`·`SMTP_PASSWORD` 중 하나라도 비면 기존과 같이 로그 스텁으로 떨어진다(팀원 로컬·CI 에 메일 계정을 나눠 주지 않으므로 기동을 막지 않는다). 실제 발송이 켜지면 `local` 이어도 로그에 코드를 남기지 않는다 |
