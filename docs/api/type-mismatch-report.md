...
1. 이 문서를 함께 검토하고 4-1, 4-2 방향에 합의
2. 프론트에서 앱 내부 충돌 정리 (4-2) — API와 무관하게 지금 가능
3. 값 표기 통일 (4-1)
4. 그 뒤 도메인별 API 명세서 작성

3번까지 끝나야 명세서의 요청·응답 예시를 확정할 수 있습니다.
지금 명세서를 먼저 쓰면 어느 쪽 값을 적어야 할지 정할 수 없습니다.
