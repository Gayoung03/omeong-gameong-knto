...
작성일: 2026-08-12 · 상태: **설계 보류 — 기능 정의 필요**
...
다른 9개 도메인과 달리 **근거로 삼을 DB 테이블이 없습니다.**
엔드포인트 스텁 파일(`app/api/v1/endpoints/guides.py`)만 있고 내용은 docstring 한 줄뿐입니다.

앱 쪽도 비어 있습니다.

```text
apps/mobile/src/features/travel-guides/    .gitkeep 만 있음
apps/mobile/app/guides/                     .gitkeep 만 있음
```
...
[`homeMockData.ts:55`](../../apps/mobile/src/features/home/data/homeMockData.ts)에 주석이 있습니다.
...
세 가지 해석이 가능하고, 각각 필요한 DB 설계가 다릅니다.

| 해석 | 내용 | 필요한 것 |
| --- | --- | --- |
| A. 운영자가 쓰는 콘텐츠 | 팀이 직접 만든 큐레이션 글 | `guides` 테이블 (제목·본문·이미지·공개일) |
