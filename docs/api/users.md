...
### 탈퇴하면 챗봇 대화는 즉시 지워집니다

회원 행은 soft delete 로 남지만, **챗봇 대화(`chat_conversations`)와 그 메시지는 탈퇴
시점에 물리 삭제되며 복구할 수 없습니다.** 사적인 AI 대화(반려동물 체중 등 실측이
유입됨)는 리뷰 같은 공개 콘텐츠와 달리 보존할 근거가 없기 때문입니다
(`ai-io-column-design.md` 8.3-2 — 삭제 연쇄는 `chat_messages` 뿐임을 FK 전수로 확인).
여행·기록·리뷰 등 다른 데이터는 이 시점에 삭제되지 않습니다.
