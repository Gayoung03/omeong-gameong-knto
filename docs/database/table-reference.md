...
| `password_changed_at` | 비밀번호를 마지막으로 바꾼 시각. **이 시각 이전에 발급된 access·refresh 토큰은 전부 무효**입니다. null이면 한 번도 바꾼 적이 없다는 뜻이라 아무 토큰도 무효화하지 않습니다 |
...
비밀번호 재설정 인증 코드입니다. 발급된 6자리 코드를 서버가 기억하는 곳입니다.

| 컬럼 | 설명 |
| --- | --- |
| `user_id` | 코드를 받은 회원 |
| `code_hash` | 코드의 argon2 해시. **원문 저장 금지** — DB가 유출돼도 코드가 새지 않아야 합니다 |
| `expires_at` | 만료 시각(기본 10분). 지난 코드는 메일함이 나중에 털려도 쓸 수 없습니다 |
| `attempt_count` | 입력 시도 횟수. 상한(기본 5회)을 넘으면 코드를 폐기합니다 |
| `used_at` | 사용·폐기 시각. null이면 살아 있는 코드 |
| `created_at` | 발급 시각 (시간당 발송 상한은 `password_reset_requests`로 셉니다) |

왜 테이블이 필요한지:

- 코드를 서버가 기억하지 않으려면 그 흔적을 토큰에 실어 앱에 들려보내야 하는데, JWT
  페이로드는 **서명일 뿐 암호화가 아니라 누구나 열어볼 수 있습니다.** 6자리는 100만
  조합뿐이라 토큰을 가로챈 쪽이 오프라인에서 즉시 맞춰볼 수 있습니다.
- 무차별 대입을 막는 `attempt_count`도 서버에만 둘 수 있습니다. 클라이언트가 들고 있는
  값으로는 시도 횟수를 셀 수 없습니다.

살아 있는 코드는 회원당 최대 하나입니다 — 새 코드를 발급할 때 이전 것들의 `used_at`을
함께 채웁니다. 여러 개가 동시에 살아 있으면 맞춰볼 수 있는 조합이 그만큼 늘어납니다.

### `password_reset_requests`

비밀번호 재설정 **요청(메일 발송)** 기록입니다. 시간당 발송 상한을 세는 데만 씁니다.

| 컬럼 | 설명 |
| --- | --- |
| `user_id` | 요청을 받은 회원 |
| `created_at` | 발송 시각. 최근 1시간 행 수로 시간당 발송 상한을 잽니다 |

왜 코드 테이블과 따로 있는지:

- 발송 상한은 코드 발급 여부와 무관하게 걸려야 합니다. 소셜 계정은 코드를 발급하지
  않고 안내 메일만 나가는데, `password_reset_codes` 행으로만 세면 소셜 발송은 셀 근거가
  없어 상한을 통째로 우회당합니다(안내 메일 무제한 → 발신 도메인 평판 손상).
- 그래서 "요청 한 번 = 메일 한 통"을 로컬·소셜 공통으로 이 표에 한 행씩 남기고, 상한은
  이 행 수로 셉니다. 코드 자체(해시·시도·만료·사용)는 `password_reset_codes`가 맡습니다.
...
| `cuisine` | AI 입출력: 음식점·카페 세부 음식 종류(한식·중식·일식·양식·분식·카페·술집·간식 …). enum 아님, 카카오 로컬 `category_name` 파싱 배치가 채움 (nullable) |
