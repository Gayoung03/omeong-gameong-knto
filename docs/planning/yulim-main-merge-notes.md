...
> 직전 내용(수동 여행 생성 + 앱 5개 화면 연결)은 **PR #74 로 이미 main 에 반영되었다.**
> 2026-08-24 에 `git log --oneline origin/main..HEAD` 로 확인하고 다시 썼다.
>
> 현재 대상: **일정 추가(장소 검색)와 장소 상세를 서버에 연결 + 목데이터 4개 삭제**
>
> **앱 공통 파일을 건드리지 않았다.** 이번엔 기능 폴더 안에서만 움직였다.
...
- **장소 상세** — GET /places/{placeId}
  404 는 예외가 아니라 `null` 로 돌려 화면의 "장소를 찾을 수 없어요" 가 받습니다.
- **일정 추가(장소 검색)** — 탭 넷을 각각 붙였습니다.

  | 탭 | 서버 |
  | --- | --- |
  | 추천 | GET /places + 좌표·반경 |
  | 최근 저장 | GET /users/me/favorites |
  | 내 숙소 | GET /places + 숙소 좌표 |
  | 나만의 장소 | GET /users/me/places |

- 이것으로 **B 파트 엔드포인트 32개 중 19개가 앱에 연결**되었습니다.

### 추천 기준점 — 평균 좌표를 쓰지 않습니다

처음에는 그 날짜 일정의 평균 좌표를 기준으로 잡았는데 목록이 비었습니다.
씨앗 Day 1 이 협재 → 애월 → 성산으로 섬을 가로질러 **평균이 한라산 한복판**이었고,
반경 10km 안에 아무 장소도 없었습니다.

기준점을 **그 날짜의 마지막 일정**으로 바꾸고, 반경을 그 날짜가 퍼진 폭에 맞춰
늘어나게 했습니다(`features/trips/utils/placeSearchArea.ts`).
일정이 몰려 있으면 좁게, 섬을 가로지르면 넓게 잡힙니다.

### 삭제한 목데이터 4개

참조가 0 이 된 것만 지웠습니다.

- `trips/mocks/trips.mock.ts`
- `trips/mocks/placeCandidates.mock.ts`
- `trips/mocks/checklist.mock.ts`
- `trips/mocks/memos.mock.ts`

`places/mocks/place.mock.ts` 는 챗봇 지도 응답이 아직 참조해서 남겼습니다.

## 공통 파일 변경

**없습니다.** src/theme/·src/components/·app/\_layout.tsx·app.config.ts·
package.json·package-lock.json·.gitignore·.env.example 모두 건드리지 않았습니다.

Alembic revision 도 없습니다. 백엔드 변경이 없는 PR 입니다.

## 팀원 확인 사항

- **`PlaceDetail` 타입에서 세 필드를 지웠습니다** — `source`·`petFriendly`·`distanceKm`.
  장소 데이터가 두 벌이던 시절의 흔적이라 서버 하나가 되면서 쓸모가 없어졌습니다.
  상세 응답에는 거리가 없어(좌표 파라미터를 받지 않는 엔드포인트) 거리 칩도 함께 지웠습니다.
  이 필드를 참조하는 코드가 있으면 알려주세요.

- **일정 추가 화면이 서버 장소를 봅니다.** 씨앗이 4곳뿐이라 목록이 짧습니다.
  개수가 줄어든 것이 연결된 증거입니다.

- **장소 상세도 서버를 봅니다.** 예전 목데이터 id(`hamdeok-beach` 같은 문자열)로
  접근하면 "장소를 찾을 수 없어요" 가 뜹니다. 서버는 UUID 를 씁니다.

- **검색은 장소명만 봅니다**(서버 `name ILIKE`). 목데이터는 설명·지역·주소까지
  훑었는데 그 동작은 서버에 없습니다. 범위를 넓히려면 서버 쪽을 고쳐야 합니다.

- **'내 숙소' 탭의 빈 문구가 상황과 어긋납니다.** 숙소가 일정에 있는데 근처에
  장소가 없으면 "숙소를 먼저 일정에 담으면…" 이 뜹니다. 문구를 둘로 나눠야 하는데
  `PLACE_SOURCE_EMPTY_MESSAGES` 가 탭당 하나라 구조를 같이 봐야 합니다. 후속 작업입니다.

- **일정 추가의 '담기'는 아직 서버에 저장되지 않습니다.** 캐시에만 들어가서
  앱을 껐다 켜면 사라집니다. `POST /route-days/{routeDayId}/items` 연결이 다음 작업입니다.

- **웹에서는 내 여행 지도가 보이지 않습니다.** `TripMapView` 가 react-native-webview 를
  쓰는데 웹 미지원입니다(장소 탐색은 `KakaoPlaceMap.web.tsx` 로 이미 해결돼 있습니다).
  이번 변경과 무관한 기존 상태입니다.

## 협의가 필요한 사항

1. **분류 코드 매핑이 부족합니다. (신규)** RDS 실데이터에 `pet_service`·`walking_trail`
   같은 코드가 있는데 어댑터 표에 없어서 **화면에 영문 코드가 그대로 뜨고, 분류 필터에도
   안 걸립니다.** 매핑만 늘릴지, 앱 필터 칩(맛집·관광·숙소 3종)을 늘릴지 정해야 합니다.
   DB 에 실제로 어떤 코드가 있는지 목록부터 확인이 필요합니다.

2. **리뷰 화면의 서버 연동 주체. (신규)** 리뷰 화면(PR #81)이 아직 목데이터를 봅니다.
   서버 API 5 개는 준비돼 있습니다 — `GET/POST /places/{placeId}/reviews`,
   `GET /users/me/reviews`, `PATCH/DELETE /reviews/{reviewId}` (`docs/api/reviews.md`).
   연결만 하면 다섯 개가 한 번에 붙습니다. 누가 붙일지 정하면 좋겠습니다.

3. **RDS 실데이터의 출처. (신규)** 씨앗에 없는 장소(개미남·금오름·놀자개 등)가
   공용 RDS 에 들어 있습니다. 누가 언제 무엇을 넣었는지 공유가 필요합니다 —
   공모전 출품작이라 사진·설명의 저작권 확인이 필요합니다.

4. **추천 방식(규칙 vs AI).** 남은 엔드포인트 3 개가 전부 여기서 막혀 있습니다.
   규칙 기반이면 취향 단어와 장소 태그를 통일해야 하고, AI 기반이면 `app/rag/` 로 갑니다.
   명세의 `POST /route-requests` → `GET /routes/{id}/status` → `regenerate` 구조 자체가
   오래 걸리는 작업을 전제한 모양이라 AI 를 염두에 둔 흔적으로 보이는데, 확정된 적은 없습니다.

5. **서버 region 값과 앱 지역 칩 통일.** (기존) 못 맞추는 값을 `null` 로 두어
   '전체' 에서만 보입니다. 지역 필터가 사실상 안 걸립니다.

6. **api-ci.yml 에 PostgreSQL.** (기존) CI 에 DB 가 없어 엔드포인트 테스트가 건너뛰어집니다.

7. **일정 날짜 이동 API.** (기존) 서버에 없어서 앱이 DELETE 후 새 날짜에 POST 합니다.
   추천 점수·추천 이유가 따라가지 못합니다.

8. **PlaceCategory 이름 충돌과 라벨 두 벌.** (기존) 내 여행은 "카페/디저트",
   장소 탐색은 "카페·식당". 이 충돌 때문에 서버 장소를 옮기는 어댑터가 둘입니다
   (`places/api/placeAdapter.ts`, `trips/api/placeCandidateAdapter.ts`).

9. **여행 이동수단이 서버 7종 / 앱 4종.** (기존)

10. **담당 영역 겹침 방지 규칙.** (기존) 이번에 `PlaceDetailScreen.tsx` 를 두 사람이
    같은 날 고쳤는데, 구역이 달라 자동 병합됐습니다. 매번 운에 기대지 않으려면
    착수 전에 서로 알리는 규칙이 필요합니다.
````
