...
> 직전 내용(무게 추정 차단과 여객선 조건부 무게 안내)은 **이미 main 에 반영되었다**(PR #261).
>
> **골격은 `.github/pull_request_template.md` 를 그대로 따른다.** 팀원 안내(신규 라이브러리·
> 마이그레이션·`.env` 추가)는 절을 새로 만들지 말고 `## 공통 파일 변경` 아래에 적는다.
>
> 현재 대상: **여행기록 카드용 손글씨 폰트 2종 추가**(PR #264)
> 2026-09-07 기준 `origin/main` 보다 3커밋 앞서 있다.
> **신규 라이브러리 없음. 마이그레이션 없음. `.env` 추가 없음. 팀원이 따로 할 일 없음.**
> 다만 `apps/api/Dockerfile` 이 바뀌어 이미지 용량이 늘었다 — 아래 `## 공통 파일 변경` 참고.

````markdown
## 작업 내용

- 여행기록 손글씨 카드 기능에서 쓸 한글 손글씨 폰트 2종을 저장소에 넣었습니다.
- 말투(`writing_style`)마다 글씨체를 다르게 씁니다. 말투를 바꿨을 때 글씨체까지 바뀌어야
  사용자가 다른 결과물이라고 느끼기 때문입니다.
  - `jeju_dialect` → 제주한라산체
  - `dog_diary` → 개구(Gaegu) Regular
- **폰트 파일만 넣는 변경입니다.** 합성 코드는 아직 없고 별도 브랜치에서 진행합니다.

## 변경 사항

- `apps/api/assets/fonts/` 신규 (약 9.4MB)
  - `jeju-hallasan.ttf` — 제주한라산체, 제주특별자치도
  - `gaegu-regular.ttf` — 개구(Gaegu) Regular
  - `KOGL-JejuHallasan.txt`, `OFL-Gaegu.txt` — 라이선스 원문
  - `README.md` — 출처·라이선스·글리프 실측 결과
- `apps/api/Dockerfile` — `COPY assets ./assets` 한 줄 추가. 이미지에 폰트가 없으면
  합성이 실패하거나 글자가 두부(□)로 그려집니다. `.dockerignore` 는 `assets` 를 막지
  않아 손대지 않았습니다.
- `docs/planning/yulim-main-merge-notes.md` — 3번 표에 2줄, 5번 초기화

### 라이선스 — 둘 다 재배포 가능한 것으로만 골랐습니다

- **제주한라산체** — 공공누리 제1유형. 상업적·비상업적 이용 가능, 변형 등 2차적 저작물
  작성 가능, **출처 표시 의무**
- **개구(Gaegu)** — SIL Open Font License 1.1

배포처의 "유료로 양도하거나 판매하는 등 상업적 행위는 금지" 문구는 **폰트 파일 자체를
판매하지 말라**는 뜻입니다. 결과물의 상업적 이용은 공공누리 제1유형이 명시적으로
허용하며, 배포 zip 에 동봉된 라이선스 원문(`KOGL-JejuHallasan.txt`)에서 확인했습니다.

**출처 표시가 의무이므로** 공모전 제출물과 앱 오픈소스 고지에 아래를 넣어야 합니다.
`assets/fonts/README.md` 에 적어뒀습니다.
...
- 제주한라산체 — 현대 한글 음절 **11,172자 전부** 포함
- 개구 — KS X 1001 2,350자. 반려동물 이름 30종·강아지 말투 어미 16종을 검사해
  빠진 글자가 없었습니다

**다만 둘 다 옛한글(아래아)이 없습니다.** `ᄒᆞ르`·`ᄆᆞᆯ` 같은 제주어 고어 표기는
두부(□)로 그려집니다. 구현 때 문구 생성 프롬프트에서 아래아를 금지하고, 합성 직전에
폰트에 없는 글자가 있는지 검사해 있으면 다음 후보 문구를 씁니다.

### 파일명을 영문으로 바꾼 이유

배포 zip 안쪽 파일명이 한글(CP949)이라 그냥 풀면 깨집니다. 저장소 네이밍 규칙
(소문자+하이픈)에 맞추고, 한글 경로가 인코딩 오류를 내는 사고를 이미 겪었기 때문에
(CocoaPods 건) 경로에 한글을 두지 않았습니다.

## 확인 사항

- [x] base 브랜치가 올바릅니다. (`dev/yulim-main` → `main`)
- [x] 커밋 메시지가 `<type>: <내용>` 형식이고, 커밋당 목적이 하나입니다.
- [x] lint / typecheck — **실행 코드 변경이 없습니다.** 프론트 코드 0줄, 파이썬 코드
      0줄이며 바이너리·문서·Dockerfile 한 줄뿐입니다.
- [x] 동작 확인 — 부를 코드가 아직 없어 화면·API 대신 **폰트 파일 자체를 검증**했습니다.
      두 파일 모두 cmap 파싱이 정상이고 위 커버리지 수치가 그 결과입니다.
- [x] 충돌이 없습니다.
...
**다만 `apps/api/Dockerfile` 이 바뀌었습니다.** 템플릿 목록에는 없지만 백엔드 공용
파일이라 적어둡니다.

- 팀원이 따로 할 일은 **없습니다.** `npm ci` 도 `.env` 추가도 마이그레이션도 없습니다.
- 다음 배포부터 이미지에 `assets/` 가 포함되어 **용량이 약 9.4MB 늘어납니다.**
- 저장소를 받으면 폰트 바이너리 2개가 함께 내려옵니다.
