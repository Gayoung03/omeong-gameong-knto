...
> 직전 내용(여행 가이드 API · 여행 준비 가이드 화면 연결 · 챗봇 파서 이동, PR #212)은
> **이미 main 에 반영되었다.** 2026-09-03 에 `git log --oneline --no-merges origin/main..HEAD`
> 로 확인하고 다시 썼다.
>
> 현재 대상: **챗봇 대화 저장·복원 레이어(백엔드)** — PR #241 · #242 / 파일 7개
>
> **공통 파일 변경 없음. 신규 라이브러리 없음. 다만 마이그레이션이 1개 있다(`b5e07c19af23`).**
> 앱 코드는 한 줄도 바뀌지 않는다 — 기존 엔드포인트의 요청·응답 모양이 그대로다.
...
`b5e07c19af23` (`add chat conversation deleted_at`)

- **팀 RDS 는 이미 적용했습니다.** 따로 하실 것 없습니다
- **로컬은 pull 후 `make backend-local-up`** 하면 자동으로 적용됩니다
- 컬럼 추가라 기존 코드를 깨지 않습니다. 다만 **새 코드는 컬럼이 없으면 대화 조회가
  실패**하므로, 로컬에서 챗봇을 쓰신다면 위 명령을 먼저 돌려주세요

## 이번 PR 내용

챗봇에 "이전 대화 목록"을 붙이기 위한 **서버 쪽 저장·복원 레이어**입니다.
대화·메시지 테이블과 엔드포인트 7개는 이미 있었지만, 목록으로 쓰기엔 구멍이 둘 있었습니다.

### 대화 목록 정렬이 실제로 "최근 순"이 됐습니다

메시지는 `chat_messages` 에 들어가므로 대화 행은 저절로 바뀌지 않습니다.
`updated_at` 의 `onupdate=now()` 는 대화 행을 UPDATE 할 때만 동작하는데, 그런 일이 생기는
것은 **제목을 짓는 첫 질문 때 한 번뿐**이라 목록이 사실상 "첫 질문 순서"로 굳어 있었습니다.
두 번째 질문부터는 아무리 대화를 이어가도 자리가 움직이지 않습니다.

`chat_access.touch_conversation()` 으로 질문·답변 저장과 **같은 트랜잭션**에서 갱신합니다.
나눠 커밋하면 답변이 끊긴 대화가 목록 아래에 남아, 사용자가 방금 물어본 대화를 못 찾습니다.

### 삭제가 소프트 삭제가 됐고 되살릴 수 있습니다

원래 DELETE 는 물리 삭제라 `chat_messages` 가 `ON DELETE CASCADE` 로 함께 사라졌습니다.
설계 결정 D3("보관 기간: 지우지 않음")과 어긋납니다. 사용자가 대화를 지우는 것은
"안 보이게 해달라"이지 "기록을 없애달라"가 아니라고 보고 `deleted_at` 만 채웁니다.

- `POST /chat/conversations/{id}/restore` 신규
- `GET /chat/conversations?deleted=true` — 휴지통. **응답 스키마가 목록과 같아** 앱이
  목록 화면 코드를 그대로 재사용할 수 있습니다
- 대화 개수 상한(100)은 **살아 있는 대화만** 계산합니다. "안 쓰는 대화를 지워주세요"라고
  안내하므로 지우면 실제로 자리가 나야 합니다
- 지운 대화는 없는 것처럼 동작합니다 — 상세·제목 수정·재삭제·메시지 목록·질문 전송이
  모두 404. 단 **소유권을 먼저** 확인하므로 남의 대화는 살아 있든 지워졌든 똑같이 403입니다

**복구할 때 `updated_at` 은 건드리지 않습니다.** 갱신하면 되살린 대화가 목록 맨 위로
튀어 오르는데, 기대하는 동작은 "원래 있던 자리로 돌아오는 것"입니다.

### 앱 코드는 바뀌지 않습니다

기존 엔드포인트의 요청·응답 모양이 그대로라, 지금 챗봇 화면은 수정 없이 그대로 돕니다.
새로 생긴 목록·휴지통·복구는 **아직 아무도 부르지 않습니다** — 앱 화면은 다음 PR 입니다.

## 공통 파일 변경

없습니다. `apps/api` 와 챗봇 문서만 바꿨습니다.

## 팀원 확인 사항

- **로컬에서 챗봇을 쓰신다면 `make backend-local-up` 을 먼저 돌려주세요.** 위 마이그레이션
  안내를 봐주세요
- **챗봇 파트 파일만 건드렸습니다** — `endpoints/chatbot.py`, `services/chat_access.py`,
  `db/models/community.py`(ChatConversation 부분), `tests/test_chatbot.py`,
  `tests/test_chat_answer.py`. 같은 파일을 작업 중이시면 미리 알려드립니다
- `docs/api/chatbot.md` 의 DELETE 절이 바뀌었고 restore 절이 새로 생겼습니다

## 협의가 필요한 사항

- **휴지통 목록에 "언제 지웠는지"가 안 나갑니다.** `ConversationItem` 을 일반 목록과
  공유하는데 거기 `deletedAt` 이 없어서입니다. 필드를 추가하면 일반 목록 응답에도 항상
  `null` 로 따라붙습니다. "3일 전 삭제" 같은 표시가 필요해지면 그때 넣는 것이 나아 보여
  이번엔 뺐습니다 — 프론트 작업 때 다시 판단합니다
- **영구 삭제 버튼은 만들지 않았습니다.** D3 이 "지우지 않음"이므로 휴지통은 계속 쌓이기만
  합니다. 의도한 동작이지만, 나중에 정리 정책이 필요하면 안건으로 올려주세요
````
