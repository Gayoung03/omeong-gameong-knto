...
> 직전 내용(카드 파이프라인 DB·S3·API 연결)은 **이미 main 에 반영됐다.** 그래서 비우고
> 이번 작업으로 다시 썼다.
>
> **골격은 `.github/pull_request_template.md` 를 그대로 따른다.** 팀원 안내(신규 라이브러리·
> 마이그레이션·`.env` 추가)는 절을 새로 만들지 말고 `## 공통 파일 변경` 아래에 적는다.
>
> 현재 대상: **여행가이드 상세의 공식 안내 링크(PR #342·#343) + 가이드 출처 URL 백필 스크립트**
> **신규 라이브러리·마이그레이션·`.env` 추가 모두 없음.** 팀원이 따로 할 일이 없다.

````markdown
## 작업 내용

여행가이드 상세 화면에서 **항공사·여객선 공식 안내 페이지로 바로 갈 수 있게** 했습니다.
규정은 자주 바뀌기 때문에 앱은 요약만 보여주고, 정확한 내용은 각 회사 홈페이지에서
확인하도록 안내합니다.

출처 주소는 **DB 가 아니라 앱에 넣었습니다.** 운영 DB 의 `guide_document_sources.source_url`
은 아직 비어 있습니다. 앱은 **API 주소가 있으면 그걸 먼저 쓰고, 없을 때만 앱에 넣은 주소**를
쓰므로 나중에 DB 가 채워져도 앱을 고칠 필요가 없습니다.

## 변경 사항

**앱 (`apps/mobile/src/features/travel-guides/`)**

- `constants/officialSources.ts` (신규) — 항공사 7곳 + 여객선 3곳(한일고속페리·씨월드고속훼리·
  오션비스타제주) 공식 안내 주소. 아리온제주는 홈페이지에 반려동물 안내가 없어 넣지 않았습니다
  (버튼이 나오지 않고 안내 문구만 보입니다).
- `api/guidesApi.ts` — `resolveOfficialSource()`: API 주소 우선, 없으면 앱 내장 주소.
- `constants/travelGuideContent.ts` — `TransportGuide` 에 `officialUrl`·`officialUrlHint` 추가.
- `screens/TravelGuideDetailScreen.tsx`
  - 상단 안내 문구: "규정은 자주 바뀌어요. 정확한 정보는 ○○ 홈페이지에서 꼭 확인해 주세요."
  - 출처 카드 아래 "○○ 공식 안내 보기" 버튼
  - 제주항공(「도움이 필요한 고객」 페이지 안의 반려동물 항목)·티웨이(사명 변경으로
    트리니티항공 사이트로 연결)는 버튼 아래 안내 한 줄

**백엔드 (`apps/api/scripts/`)**

- `seed_guides.py` — 항공사 7곳 출처 URL 을 시드에 채움.
- `backfill_guide_source_urls.py` (신규) — 이미 적재된 DB 에 URL 을 채우는 스크립트.
  기본은 미리보기(dry-run)이고 이미 다른 값이 있으면 덮어쓰지 않습니다.
  **아직 운영 DB 에 돌리지 않았습니다** — 앱이 주소를 들고 있어 당장은 필요 없습니다.

## 확인 사항

- [x] base 브랜치가 올바릅니다. (`dev/yulim-main` → `main`)
- [x] 커밋 메시지가 `<type>: <내용>` 형식이고, 커밋당 목적이 하나입니다.
- [x] `npm run lint` / `tsc --noEmit` 를 통과했습니다.
- [x] 화면 동작을 직접 확인했습니다. (상세 화면 안내 문구·링크 버튼)
- [x] 충돌이 없습니다. (최신 main 을 합친 뒤 올립니다)

## 공통 파일 변경

- [x] 해당 없음

신규 라이브러리·마이그레이션·`.env` 추가가 없어 팀원이 따로 할 일은 없습니다.
...
| 2026-09-20 | **main PR 준비 — 5번을 여행가이드 공식 안내 링크 건으로 다시 썼다.** 직전 내용(카드 파이프라인 DB·S3·API 연결)은 이미 main 에 반영돼 있었다(`generation_message` 가 `origin/main` 에 있음). 이번 대상은 PR #342·#343(상세 화면 링크·안내 문구)과 9/15 의 출처 URL 백필 스크립트(`4d2e227`)다. **출처 URL 을 DB 가 아니라 앱(`officialSources.ts`)에 넣은 이유** — 운영 DB 의 `source_url` 이 비어 있고(RDS 접속 timeout 으로 백필 미실행), 앱 심사 이후라 앱 쪽에서 처리하기로 했다. API 값이 있으면 그쪽이 우선이라 나중에 DB 가 채워져도 앱 수정은 필요 없다. 공통 파일 변경·신규 라이브러리·마이그레이션 모두 없음 |
