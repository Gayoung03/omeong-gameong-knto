...
> 직전 내용(여행기록 카드용 손글씨 폰트, PR #264)은 **아직 main 에 안 올라갔다** —
> `dev/yulim-main` 까지만 갔다. 그래서 이번 main PR 에 폰트까지 함께 담는다.
>
> **골격은 `.github/pull_request_template.md` 를 그대로 따른다.** 팀원 안내(신규 라이브러리·
> 마이그레이션·`.env` 추가)는 절을 새로 만들지 말고 `## 공통 파일 변경` 아래에 적는다.
>
> 현재 대상: **여행기록 손글씨 카드 파이프라인 + 손글씨 폰트 2종**
> 2026-09-13 기준 `origin/main` 보다 7커밋 앞서 있다.
> **신규 라이브러리 1개(`pillow`). 마이그레이션 없음. `.env` 추가 없음.**
> 팀원은 `apps/api` 에서 **`uv sync` 한 번**만 돌리면 된다.

````markdown
## 작업 내용

여행 사진 한 장으로 **손글씨 메모가 얹힌 여행기록 카드**를 만드는 백엔드 파이프라인입니다.
말투를 두 가지로 고를 수 있습니다 — 제주 방언, 반려동물 1인칭 일기.

**이 단계에서는 DB·S3·API 라우트를 붙이지 않았습니다.** 로컬 사진 파일 하나를 넣으면
카드 PNG 가 나오는 데까지이고, **앱과 API 는 이 코드를 아직 부르지 않습니다.**
프롬프트를 눈으로 맞추는 일이 먼저라서 연결은 다음 PR 로 미뤘습니다.

### 결정 하나를 남깁니다 — 사진 위에 직접 그리게 하면 안 됩니다

`images.edit` 는 원본 위에 덧그리는 것이 아니라 **모든 픽셀을 새로 만듭니다.** 그래서
"원본을 그대로 두라"는 지시는 모델이 구조적으로 지킬 수 없습니다. 실제로 사람은 물론
반려동물 얼굴까지 바뀌었습니다(눈이 커지고 얼굴이 갸름해지는 쪽). 여행 **기록** 기능에서
이건 미관 문제가 아니라 기능 실패입니다.

그래서 모델에게는 **검은 바탕에 흰 손글씨만 그린 레이어**를 받고, 그것을 원본 사진에
우리가 얹습니다. **원본 사진은 한 픽셀도 바뀌지 않습니다.**

## 변경 사항

- `apps/api/app/integrations/llm/travel_card/` 신규 — 다섯 단계

      vision       사진 분석 + 안전 판정      (LLM)
      caption      메모 4~6개 생성            (LLM)
      prompts      이미지 프롬프트 조립       (문자열 합치기, 호출 없음)
      image_edit   손글씨 레이어 생성         (LLM, 전체 지연의 대부분)
      card_render  원본에 합성                (Pillow, 호출 없음)

- **예외를 던지지 않습니다.** 실패도 결과로 봅니다. `CardResult.outcome` 다섯 가지
  (`OK` / `BLOCKED_INPUT` / `UNREADABLE_IMAGE` / `BLOCKED_OUTPUT` / `FAILED`)로 갈라
  돌려주어 프론트가 분기 하나로 처리할 수 있게 했습니다. 안내 문구는 **사진을 탓하지 않는
  톤**으로 씁니다 — 음식·동물 클로즈업에서 안전 판정 오탐이 잦습니다.
- **안전 판정은 애매하면 차단입니다.** JSON 파싱 실패, `safe` 누락, 문자열 `"true"`,
  정수 `1` 이 전부 차단으로 떨어집니다. 통과가 기본값이면 판정이 없는 것과 같습니다.
- **업로드 전에 사진을 한 번 다시 굽습니다.** 겉만 JPEG 인 휴대폰 사진(**MPO** — 듀얼
  카메라가 JPEG 두 장을 한 파일에 담는 형식)이 OpenAI 에서 `invalid_image_file` 로
  거절되던 문제를 막습니다. EXIF 회전, CMYK, 색 프로파일도 여기서 정리됩니다.
- `apps/api/assets/fonts/` 신규 (약 9.8MB) — 합성 실패 시 폰트로 직접 쓰기 위한 대비이자
  말투별 글씨체 기준입니다. `jeju-hallasan.ttf`(제주한라산체), `gaegu-regular.ttf`(개구),
  라이선스 원문 2개, `README.md`
- `apps/api/Dockerfile` — `COPY assets ./assets` 한 줄. 없으면 프로덕션에서 글자가
  두부(□)로 그려집니다
- `apps/api/scripts/travel_card_smoke.py` — 확인용 CLI. 사진 경로를 주면 카드가 나옵니다
- `apps/api/scripts/travel_card_recompose.py` — **합성만** 다시 돌립니다(API 호출 없음).
  카드 한 장에 약 66원이 나가서, 눈금 하나 바꿀 때마다 재생성하지 않으려고 만들었습니다
- 테스트 73개 신설 — 안전 판정, EXIF 회전, MPO, 한글 깨짐 가드, 합성 마스크

### 라이선스 — 출처 표시가 의무입니다

- **제주한라산체** — 공공누리 제1유형. 상업적 이용·변형 가능, **출처 표시 의무**
- **개구(Gaegu)** — SIL Open Font License 1.1

공모전 제출물과 앱 오픈소스 고지에 아래를 넣어야 합니다(`assets/fonts/README.md` 에도
적어뒀습니다).
...
**두 폰트 모두 옛한글(아래아)이 없습니다.** `ᄒᆞ르`·`ᄆᆞᆯ` 같은 제주어 고어 표기는
두부(□)로 그려지므로 문구 생성 프롬프트에서 아래아를 금지해 두었습니다.
...
- [x] **앱 기동에 영향이 없는지 확인했습니다.** `travel_card` 를 임포트하는 코드가 파이프라인
      폴더 밖에 하나도 없고, `OPENAI_API_KEY` 없이도 `app.main` 이 정상 임포트됩니다.
- [x] 충돌이 없습니다.

## 공통 파일 변경

- [x] 해당 없음 (프론트 기준 — 앱 파일은 한 줄도 건드리지 않았습니다)

**백엔드 공용 파일 2개가 바뀌었습니다.**

- `apps/api/pyproject.toml` — **`pillow>=11,<13` 추가.** 합성 단계가 씁니다.
  **팀원은 `apps/api` 에서 `uv sync` 를 한 번 돌려야 합니다.**
- `apps/api/Dockerfile` — `COPY assets ./assets`. 다음 배포부터 이미지 용량이
  **약 9.8MB 늘어납니다.**

마이그레이션 없음. `.env` 추가 항목 없음(`OPENAI_API_KEY` 는 이미 쓰고 있는 값입니다).
