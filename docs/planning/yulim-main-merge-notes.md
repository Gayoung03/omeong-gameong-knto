...
| `pillow` (백엔드)                        | >=11,<13 | 여행기록 카드 합성 — 생성 이미지에서 손글씨 획만 뽑아 원본 사진에 다시 얹는다 | #297    |
...
| `.env.example`                                                        | **`TRAVEL_CARD_*` 4개 추가**(이미지 모델·품질·분석·문구). 넷 다 기본값이 있어 **적지 않아도 동작한다** — 값의 존재와 **비용(기록 1장당 약 66원)** 을 알리기 위한 항목이다. 키는 기존 `OPENAI_API_KEY` 를 그대로 쓴다 | 예정   |
...
cd ../api && uv sync              # pillow (#297 에서 추가됨)
cd ../.. && make db-migrate-local  # 로컬 DB 를 쓰는 사람
```

**마이그레이션(`travel_logs.generation_message`)을 안 돌리면 여행기록 API 가 500 을
낸다** — 코드에 있는 컬럼이 DB 에 없기 때문이다. 컬럼 추가 하나라 잠금·되돌리기
부담은 없다.

> ⚠️ **`uv run alembic upgrade head` 를 직접 치지 말 것.** 그 명령은 `.env` 의
> `DATABASE_URL`(앱 계정 `omeong_app`)로 접속하는데, 그 계정은 테이블 주인이 아니라
> **`must be owner of table ...` 로 실패한다.** 마이그레이션은 `MIGRATION_DATABASE_URL`
> (`omeong_admin`)을 쓰는 `make` 타깃으로만 돌린다 — 그러라고 계정이 나뉘어 있다.
>
> **공유 RDS 는 팀원이 각자 돌리는 것이 아니다.** README 대로 **시점을 합의한 뒤
> 한 사람이** `make db-migrate-check` 를 한 번 돌리면 전원이 해결된다
> (`upgrade head` → `current --check-heads` → `alembic check` 까지 검증한다).
...
> 직전 내용(카드 파이프라인 + 손글씨 폰트)은 **PR #297 로 main 에 반영됐다.** 그래서 비우고
> 이번 작업으로 다시 썼다.
>
> **골격은 `.github/pull_request_template.md` 를 그대로 따른다.** 팀원 안내(신규 라이브러리·
> 마이그레이션·`.env` 추가)는 절을 새로 만들지 말고 `## 공통 파일 변경` 아래에 적는다.
>
> 현재 대상: **여행기록 카드 파이프라인을 DB·S3·API 에 연결**
> **신규 라이브러리 없음**(`httpx`·`boto3`·`pillow` 모두 이미 있다).
> **마이그레이션 1건**(`travel_logs.generation_message`). `.env` 는 추가 항목이 있지만
> **전부 기본값이 있어 적지 않아도 된다.**
> 공유 RDS 는 **합의 후 한 사람이 `make db-migrate-check`** 를 한 번 돌리면 된다.
> 로컬 DB 를 쓰는 팀원은 `make db-migrate-local`.

````markdown
## 작업 내용

여행기록 카드 파이프라인을 **앱이 실제로 쓸 수 있게** 연결했습니다.

지금까지 `travel_log_image.py` 는 **원본 사진 주소를 그대로 돌려주는 임시 구현**이었습니다.
카드를 만드는 코드(`travel_card/`)는 #297 로 main 에 있었지만 아무도 부르지 않아서,
앱에서 기록을 만들면 손글씨 카드가 아니라 **올린 사진이 그대로** 결과물로 나왔습니다.
그 사이를 이었습니다.

    원본 키 읽기 --> build_card() --> 결과 PNG 올리기 --> 공개 주소

함께 **실패했을 때 "왜"를 앱까지 내려보냅니다.** 상태(`failed`)만으로는 *다른 사진을
고르세요* 와 *잠시 후 다시 하세요* 가 구분되지 않았습니다. 앞엣것은 같은 사진으로 다시
눌러도 또 실패하는데, **재시도 한 번마다 카드 한 장 값(약 66원)이 나갑니다.**

## 변경 사항

**연결**

- `integrations/llm/travel_log_image.py` — 임시 구현을 실제 파이프라인 호출로 교체.
  배치는 `loose`, 손글씨는 레이어 방식(`ink_plate`)으로 고정했습니다 — 둘 다 이미
  결론이 난 값이라 인자로 받지 않습니다.
- `integrations/storage.py` (신규) — S3 읽기·쓰기. **원본을 URL 로 내려받지 않고
  공개 주소에서 키만 떼어 버킷에서 직접 읽습니다.** 앱이 보낸 주소로 서버가 HTTP 요청을
  내보내는 통로를 만들지 않기 위해서고, CloudFront 를 비공개로 잠가도 계속 동작합니다.
  호스트 비교에 `startswith` 를 쓰지 않습니다(`https://<허용호스트>@evil.test` 가 통과합니다).
- `endpoints/travel_logs.py` — 뒷작업이 **장소·날짜·반려동물 이름을 카드 문구에 넘깁니다.**
  이름은 **한 마리일 때만** 넘깁니다 — 두 마리 사진에 한 이름만 주면 메모가 엉뚱한 쪽을
  부릅니다. 예상 못 한 예외까지 전부 잡습니다(새면 상태가 `generating` 에 영영 멈춥니다).
- 완성 카드는 `travel-log-card/` 접두사로 올라갑니다(원본은 `travel-log/`).
  원본과 결과물은 보관·정리 기준이 달라질 수 있어 나눠 두었습니다.

**실패 사유 전달**

- `travel_logs.generation_message` 컬럼 추가(nullable). 성공하면 비웁니다 —
  남겨두면 완성된 기록에 지난 실패 안내가 붙습니다.
- `GET /travel-logs/{logId}/status` 응답에 `generationMessage` 추가.
- 문구의 정본은 `travel_card/agent.py` 의 `MESSAGES` 하나입니다.
  **`CardResult.message` 를 화면에 쓰지 않습니다** — 거기엔 타임아웃·HTTP 코드 같은
  기술적 사유가 괄호로 붙어 있어 사용자 화면에 그대로 나갑니다. 그쪽은 로그로만 남깁니다.

**앱**

- 실패 화면이 서버 문구를 그대로 보여줍니다(없으면 기존 기본 문구).
- **사진 상한 15MB → 10MB.** 서버가 10MB(`uploads.py`·`travel_card/config.py`)여서
  10~15MB 사진은 **화면을 통과하고 업로드에서 413** 으로 떨어지고 있었습니다.
- **폴링 제한 2분 → 5분.** 서버 타임아웃 합계(30+30+180초)보다 짧아서, 서버가 아직
  만들고 있는데 화면만 실패로 바뀔 수 있었습니다 — 돈은 나가고 사용자는 못 받습니다.
- 빠진 항목이 있어 못 보낸 경우의 문구를 분리했습니다. "잠시 후 다시"는 그 경우에
  아무것도 고쳐주지 않습니다.

## 확인 사항

- `ruff check` · `ruff format` 통과, `tsc --noEmit` 통과.
- 테스트: 저장소 주소→키 변환 8건 신규(`test_storage.py` — `@evil.test` 우회·경로
  탈출·대소문자·설정 없음), 실패 사유 전달 3건 신규.
- **테스트에서 카드 생성은 대역으로 막았습니다**(`conftest.py` 의 `_fake_travel_log_card`).
  막지 않으면 여행기록 테스트 전체가 매번 돈을 씁니다. `_disable_openai` 로는 막히지
  않습니다 — 카드 쪽은 별도 Settings 를 읽습니다.
- 실제 사진으로 앱에서 한 장 만들어 카드가 나오는 것까지 확인해야 끝납니다(RDS·S3 필요).

## 공통 파일 변경

- `.env.example` — `TRAVEL_CARD_*` 4개. **전부 기본값이 있어 적지 않아도 동작합니다.**
  값의 존재와 **비용(기록 1장당 약 66원, "다시 만들기"도 같은 값)** 을 알리기 위한 항목입니다.
  키는 기존 `OPENAI_API_KEY` 를 그대로 씁니다.
- **마이그레이션 1건** — `travel_logs.generation_message`(nullable 컬럼 추가).
  안 돌리면 여행기록 API 가 500 을 냅니다. Alembic head 는 1개로 유지됩니다.
  - 공유 RDS: **시점을 합의한 뒤 한 사람이** `make db-migrate-check` 를 한 번.
  - 로컬 DB: 각자 `make db-migrate-local`.
  - `uv run alembic upgrade head` 를 직접 치면 앱 계정(`omeong_app`)으로 붙어
    **`must be owner of table travel_logs`** 로 실패합니다. 마이그레이션 계정은
    `MIGRATION_DATABASE_URL`(`omeong_admin`)이고, `make` 타깃이 그것을 씁니다.
...
> - **PR 본문에 표를 넣을 때는 표 위 빈 줄이 살아 있는지 확인한다.** 붙여넣기에서 빈 줄이
>   사라지면 표가 통째로 깨진다. 목록으로 쓸 수 있으면 목록이 안전하다.
...
| 2026-09-03 | 챗봇 대화 저장·복원 레이어(백엔드) — PR #241·#242 를 `dev/yulim-main` 에 반영. 5번 항목을 이번 기준으로 다시 썼다(직전 PR #212 내용은 이미 main 에 있다). **마이그레이션 `b5e07c19af23` 1개** — 팀 RDS 는 적용 완료, 로컬은 `make backend-local-up` 으로 자동 적용된다. 공통 파일 변경·신규 라이브러리 없음 |
...
| 2026-09-07 | **여행기록 카드용 손글씨 폰트 2종을 저장소에 추가.** 제주 방언 문구는 제주한라산체, 강아지 일기는 개구(Gaegu)를 쓴다 — 말투를 바꾸면 글씨체까지 바뀌어야 사용자가 다른 결과물이라고 느낀다. **라이선스를 둘 다 재배포 가능한 것으로만 골랐다** — 제주한라산체는 공공누리 제1유형(상업적 이용·변형 가능, **출처 표시 의무**), 개구는 SIL OFL 1.1. 배포처의 "유료로 양도·판매하는 상업적 행위 금지" 문구는 폰트 파일 자체를 팔지 말라는 뜻이고, 결과물의 상업적 이용은 공공누리 제1유형이 명시적으로 허용한다(라이선스 배지 원문 확인). **글리프 커버리지를 실측했다** — 제주한라산체는 현대 한글 11,172자 전부, 개구는 KS X 1001 2,350자로 실사용 한국어를 전부 덮는다(반려동물 이름 30종·강아지 말투 어미 16종 검사 통과). 다만 **둘 다 옛한글 아래아가 없어** `ᄒᆞ르` 같은 제주어 고어 표기는 두부(□)가 된다 — 문구 생성 프롬프트에서 아래아를 금지하고 합성 직전에 글자 존재 여부를 검사한다. 파일명은 한글 경로 인코딩 사고를 피하려고 영문 소문자+하이픈으로 바꿨다. 신규 라이브러리 없음 |
| 2026-09-07 | **머지 노트 5번을 폰트 건(PR #264)으로 다시 씀.** 직전 내용(무게 추정 차단·여객선 조건부 무게 안내)은 PR #261 로 이미 main 에 올라갔는데 5번에 그대로 남아 있어, 팀원이 이미 반영된 안내를 다시 보게 될 상태였다(규칙 D-3). 함께 **중복돼 있던 '채울 때 메모' 블록 2개를 1개로 합쳤고**, PR 본문에 표를 넣을 때 표 위 빈 줄을 확인하라는 항목을 메모에 추가했다 — 붙여넣기에서 빈 줄이 사라져 표가 깨진 적이 있다. 문서만 변경 |

| 2026-09-13 | **여행기록 손글씨 카드 파이프라인 구현(백엔드만).** 사진 한 장 → 카드 PNG 까지 다섯 단계(vision·caption·prompts·image_edit·card_render). **DB·S3·API 라우트는 붙이지 않았다** — 프롬프트를 눈으로 맞추는 일이 먼저라 연결은 다음으로 미뤘고, `travel_card` 를 임포트하는 코드가 폴더 밖에 하나도 없어 앱 동작에 영향이 0 이다(키 없이 `app.main` 임포트까지 확인). **핵심 결정: 이미지 모델에게 사진 위에 직접 그리게 하지 않는다.** `images.edit` 는 덧그리는 게 아니라 모든 픽셀을 새로 만들어서 사람·반려동물 얼굴이 바뀐다 — 여행 '기록' 에서는 기능 실패다. 검은 바탕에 흰 손글씨만 그린 레이어를 받아 원본에 우리가 얹는다(원본 무손상). **생성물에서 손글씨를 추측해 꺼내는 방식은 두 번 시도했다가 버렸다** — 흰 털·흰 운동화·나뭇잎 틈 하늘이 손글씨로 딸려 오고, 원본과 대조해 걸러내면 이번엔 글씨가 깎였다. 어느 픽셀이 손글씨인지는 결과물에 없는 정보라 눈금으로 해결되지 않는다. **글자가 가로로 12~16% 늘어나던 것도 잡았다** — 캔버스는 1:1/3:2/2:3 셋뿐인데 사진 비율은 제각각이라, 사진을 먼저 캔버스 비율로 잘라 1:1 로 맞춘다. **MPO**(휴대폰 듀얼 카메라 사진)가 `invalid_image_file` 로 거절되던 것도 업로드 전 표준화로 막았다. **신규 라이브러리 `pillow` 1개** — 팀원은 `apps/api` 에서 `uv sync` 필요(1·3·4번 표 반영). 마이그레이션 없음. 테스트 73개 신설, `ruff` 통과·`pytest` 345개 통과. 5번을 이번 건으로 다시 썼다 — 폰트(PR #264)가 아직 main 에 안 올라가 함께 담는다 |
| 2026-09-14 | **카드 파이프라인을 DB·S3·API 에 연결.** `travel_log_image.py` 의 임시 구현(원본 주소를 그대로 돌려주던 것)을 실제 호출로 교체하고, S3 읽기·쓰기를 `integrations/storage.py` 로 신설했다 — **원본은 URL 로 내려받지 않고 공개 주소에서 키만 떼어 버킷에서 직접 읽는다**(앱이 보낸 주소로 서버가 요청을 내보내지 않게). 실패 사유를 앱까지 내려보내려고 `travel_logs.generation_message` 컬럼을 추가했다(**마이그레이션 1건 — 팀원 `alembic upgrade head` 필요**) — 상태만으로는 *다른 사진을 고르세요* 와 *잠시 후 다시* 가 구분되지 않고, 재시도 한 번마다 카드 한 장 값이 나간다. 함께 앱의 사진 상한을 15MB→10MB 로(서버는 10MB 라 그 사이 용량이 업로드에서 413 으로 떨어지고 있었다), 폴링 제한을 2분→5분으로 고쳤다(서버 타임아웃 합계 240초보다 짧아 **서버가 만들고 있는데 화면만 실패**로 바뀔 수 있었다). `.env.example` 에 `TRAVEL_CARD_*` 와 비용(장당 약 66원)을 적었다. 5번 항목은 PR #297 로 올라간 직전 내용을 비우고 이번 건으로 다시 썼다. 신규 라이브러리 없음. **마이그레이션 안내를 한 번 틀리게 적었다가 고쳤다** — `uv run alembic upgrade head` 는 앱 계정(`omeong_app`)으로 붙어 `must be owner of table travel_logs` 로 실패한다(실제로 겪었다). 마이그레이션 계정은 `MIGRATION_DATABASE_URL`(`omeong_admin`)이고 `make db-migrate*` 타깃이 그것을 쓴다. 공유 RDS 는 README 대로 **합의 후 한 사람만** 돌린다. |
