...
const STYLE_OPTIONS: { value: WritingStyle; title: string; subtitle: string; icon: keyof typeof Ionicons.glyphMap }[] = [
  { value: 'dog_diary', title: '강아지 일기', subtitle: '귀여운 강아지 말투', icon: 'paw-outline' },
  { value: 'jeju_dialect', title: '제주 방언', subtitle: '제주어로 남기는 여행 기록', icon: 'water-outline' },
];
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.safeArea}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <View style={styles.summaryCard}>
            <RemoteImage borderRadius={radius.md} style={styles.summaryImage} uri={draft.localPhotoUri ?? undefined} />
            <View style={styles.summaryInfo}>
              <Text style={styles.summaryTitle}>{draft.recordedDate?.replaceAll('-', '.')} · {draft.placeName}</Text>
          ...
          <View style={styles.optionGrid}>
            {STYLE_OPTIONS.map((option) => {
              ...
              return (
                <Pressable key={option.value} onPress={() => updateDraft({ writingStyle: option.value })} style={[styles.styleCard, selected && styles.selectedCard]}>
                  {selected ? <Ionicons color={colors.primary} name="checkmark-circle" size={20} style={styles.checkIcon} /> : null}
                  <Ionicons color={selected ? colors.primary : colors.secondary} name={option.icon} size={34} />
          ...
          <View style={styles.moodGrid}>
            {MOOD_OPTIONS.map((option) => {
              ...
              return (
                <Pressable key={option.value} onPress={() => updateDraft({ mood: option.value })} style={[styles.moodButton, selected && styles.selectedCard]}>
                  <Ionicons color={selected ? colors.primary : colors.iconGray} name={option.icon} size={21} />
                  <Text style={[styles.moodLabel, selected && styles.selectedLabel]}>{option.label}</Text>
                </Pressable>
              );
            })}
          </View>
        ...
        <View style={styles.footer}>
          <Pressable disabled={!canSubmit} onPress={generate} style={[styles.submitButton, !canSubmit && styles.submitDisabled]}>
            <Text style={[styles.submitLabel, !canSubmit && styles.submitLabelDisabled]}>이 순간 기록하기</Text>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  editButton: { borderColor: colors.secondary, borderRadius: radius.sm, borderWidth: 1, paddingHorizontal: spacing.sm, paddingVertical: 7 },
  ...
  input: { color: colors.textPrimary, flex: 1, fontSize: 14, minHeight: 76, textAlignVertical: 'top' },
  messageBox: { borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, minHeight: 112, padding: spacing.sm },
  messageMeta: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' },
  moodButton: { alignItems: 'center', backgroundColor: colors.mintBg, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexBasis: '48%', flexDirection: 'row', gap: spacing.sm, padding: spacing.sm },
  ...
  sectionTitle: { color: colors.textPrimary, fontSize: 15, fontWeight: '700', marginTop: spacing.sm },
  selectedCard: { backgroundColor: colors.orangeBg, borderColor: colors.primary },
  selectedLabel: { color: colors.primary },
  styleCard: { alignItems: 'center', backgroundColor: colors.mintBg, borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flex: 1, minHeight: 126, padding: spacing.md },
  ...
  submitButton: { alignItems: 'center', backgroundColor: colors.primary, borderRadius: radius.sm, paddingVertical: 14 },
  ...
  summaryCard: { alignItems: 'center', borderColor: colors.border, borderRadius: radius.md, borderWidth: 1, flexDirection: 'row', gap: spacing.sm, padding: spacing.sm },
