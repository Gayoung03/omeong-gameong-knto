...
def test_allow_nearby_skips_distance_but_keeps_name_check() -> None:
    ...
    assert duplicate_reason("거문오름", (33.4241, 126.9296), existing, allow_nearby=True) is None
    # 그래도 같은 이름은 여전히 막는다.
    same_name = [ExistingPlace("거문오름", (34.0, 127.0), True)]
    assert (
        duplicate_reason("거문오름", (33.4241, 126.9296), same_name, allow_nearby=True) is not None
    )
