...
class PhotoAnalysis:
    ...
    #: 사람·동물의 **머리**가 있는 칸 이름. 화면을 가로 3 × 세로 3 으로 나눈
    #: `"왼쪽-중간"` 같은 값이다(`vision.ZONES`).
    #:
    #: 얼굴을 덮지 말라는 지시는 프롬프트 네 곳에 있었지만 계속 어겼다
    #: (2026-09-13·09-14 실측). 모델이 규칙을 안 읽어서가 아니라 **머리가 어느
    #: 픽셀인지 못 맞추기 때문이다** — 누끼 테두리를 포기했던 이유와 같다.
    #: 그래서 자리를 말로 금지하는 대신 **칸 이름으로 건넨다.**
    #:
    #: **얼굴이 아니라 머리다.** 처음엔 "얼굴"로 물었는데 뒷모습만 찍힌 사진에서
    #: 빈 배열이 와 금지 구역이 통째로 안 붙었다(09-15 실측). 뒤통수 위에 글씨가
    #: 얹히는 것도 똑같이 보기 싫다. 고친 뒤 같은 사진에서 두 사람을 다 잡았다.
    #:
    #: 머리만 담는다. 몸통·배경은 글씨가 얹혀도 괜찮다(사장님 기준, 09-14).
    head_zones: list[str] = field(default_factory=list)
