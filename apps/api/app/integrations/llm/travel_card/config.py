...
공용 설정 파일을 늘리면 서버 기동 검증에 걸려, 이 기능과 상관없는 팀원의 로컬 기동에
영향이 간다.

**라우트를 붙일 때 옮기겠다고 적어 뒀지만, 붙이면서 옮기지 않기로 했다(2026-09-14).**
아래 설명대로 여기서도 같은 `.env` 를 읽으므로 동작에 차이가 없고, 배관 작업과
설정 이사를 한 PR 에 섞으면 목적이 둘이 된다. `.env.example` 에만 항목을 적어
팀원이 값의 존재와 비용을 알 수 있게 했다.
