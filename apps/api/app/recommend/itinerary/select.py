...
from app.db.models.enums import ScheduleItemType, TransportType
...
from .types import MAX_ALTERNATIVES, UNFILLED_MEAL_REASON, Coordinate, ScheduledItem, UnfilledSlot
...
def best_candidate_with_diversity(
    ...
    *,
    ...
    if not enforce_diversity:
        ...
        rungs = (
            Rung(frozenset(), False, VERIFIED_ONLY, ctx.max_travel_min),
            Rung(frozenset(), False, VERIFIED_ONLY, None),
            Rung(frozenset(), False, ANY_TIER, None),
        )
    ...
    rungs = (
        Rung(blocked, True, VERIFIED_ONLY, ctx.max_travel_min),
        Rung(frozenset(), True, VERIFIED_ONLY, ctx.max_travel_min),
        Rung(frozenset(), False, VERIFIED_ONLY, ctx.max_travel_min),
        Rung(frozenset(), False, VERIFIED_ONLY, None),
        Rung(frozenset(), False, ANY_TIER, None),
    )
...
def top_alternatives(
    ...
    rung = Rung(frozenset(), False, ANY_TIER, None)
